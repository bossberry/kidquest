...
- **Emoji-free UI (2026-06-11)**: All emoji removed from UI across Home.jsx, Collection.jsx, BottomNav.jsx, WorldScreen.jsx, TreasureSlot.jsx, Report.jsx, CreatureDetailPopup.jsx, HatchOverlay.jsx (toast only). Remaining intentional emoji: TreasureSlot ITEMS array (slot symbols), BattleScreen creature displays, mini-game egg visuals. HatchOverlay creature reveal now uses `CreatureCanvas` (emoji replaced in Phase 5 birth sequence).
...
### Creature System — Element + Evolution + Bond (2026-06-15)

- **`src/lib/creatureSystem.js`** (NEW): `determineElement(xpThai, xpMath, xpEng, accuracy, streak)` → element from `{fire, water, thunder, nature, shadow, light}`. Logic: acc≥90% + streak≥14 → light; acc≥85% → nature; streak≥7 + dominant<100XP → shadow; else dominant-subject map (thai→fire, math→water, eng→thunder). `CREATURE_ELEMENT_COLORS` (Fire=#E24B4A, Water=#378ADD, Thunder=#EF9F27, Nature=#639922, Shadow=#534AB7, Light=#FAC775). `CREATURE_ELEMENT_NAMES_TH`. `getEggElementHint(...)` returns element hint for stage 2+ display. `calcEvoStage(battleLevel, playerTier, bondMeter, currentEvoStage)` → baby→teen (lv≥11+tier≥2) → final (lv≥26+tier≥5+bond≥60). `EVO_STAGE_LABELS_TH`.
- **`src/config/gameConfig.js`**: Added `CREATURE_LEVELS = { xpPerLevel:80, maxLevel:50, evoThresholds:{ teen:{minLevel:11,minTier:2}, final:{minLevel:26,minTier:5,minBond:60} } }`.
- **`src/lib/state.js`**: Added `bossDefeatedThisTier: false` to `defaultState()`. Added import + backfill migration in `_migrateBattleStats` for all existing creatures: adds `element`, `evoStage`, `bondMeter:0`, `bornAtk/Def/Spd`, `bornCrit`, `bornDate`, `bornTier`, `creatureName:null`.
- **`src/context/StateContext.jsx`**: Imports `determineElement`/`calcEvoStage` from creatureSystem. New ACTIONS: `SET_CREATURE_NAME`, `ADD_CREATURE_BOND`, `CREATURE_EVOLVE`. **HATCH_COMPLETE** now stores `element`, `evoStage:'baby'`, `bondMeter:0`, `bornAtk/Def/Spd/Crit`, `bornDate`, `bornTier`, `creatureName:null` on each new egg. **ADD_XP** now distributes creature XP: `party[0]` (active) = 100% of earned XP as battleXP, `party[1+]` (bench) = 50%; evolution checked after each gain. **ROUND_COMPLETE** adds +2 bond to active creature + evolution check. **INCREMENT_BATTLE_WINS** adds +1 bond to active creature + evolution check. **DEFEAT_BOSS** now sets `bossDefeatedThisTier: true`.
- **`src/components/HatchOverlay.jsx`**: After 5-tap hatch reveal, shows element badge (colored pill) + two buttons: "ตั้งชื่อ" → 'naming' phase or "ข้ามการตั้งชื่อ". Naming phase: 5 large Thai name tap targets from `CREATURE_NAME_SUGGESTIONS[element]`. "ข้ามการตั้งชื่อ" available in both done and naming phases. **Phase 5 birth sequence** (2026-06-15): `buildCreatureDNA(buildEggStats(state))` called synchronously at reveal time → `setCreatureDNA(dna)`. `CreatureCanvas` replaces emoji in 'done'/'naming' phases: 150px, `idleMode='celebrate'` during 'done', element-color `drop-shadow` glow filter. `playCreatureSound(buildVoiceProfile(dna), 'celebrate')` fired at reveal. Birth-pop CSS keyframe (`creature-birth` + `.hatch-creature-enter`) for scale 0.15→1.14→1.0 spring animation.
- **`src/components/Home.jsx`**: When creature exists, the center zone shows: large 160×160 pixel-art canvas (`drawCreature` from `creatureAlgorithm.js`, stats include `evoStage` for correct stage), creature name (large gold), Lv.X badge, compact single-line stat row (ATK/DEF/SPD/HP color-coded small font). Below: scrollable party portrait bar — 56×56 canvas per creature, name + Lv underneath, active = gold `#EF9F27` border + glow, tap dispatches `SET_ACTIVE_CREATURE`. Replaces the old 2×2 ATK/DEF/SPD/HP stat grid and Party HP bar section. Header still shows element hint badge (egg only). Imports: `getEggElementHint`, `CREATURE_ELEMENT_COLORS`, `EVO_STAGE_LABELS_TH`, `drawCreature`, `getCreatureSeed`.
- **`src/context/StateContext.jsx`** (2026-06-15 addition): `SET_ACTIVE_CREATURE` action — reorders `state.party[]` so the chosen creature moves to index 0 (becomes battle-active). No-op if not in party or already active.
- **`src/components/Collection.jsx`** (2026-06-15): `creatureName(egg)` module-level helper (custom name > species name > fallback). PartyGrid "ทีม" tab: active creature (party[0]) shows gold "★ ตัวหลัก" badge at card top; bench members show "★ ตั้งเป็นตัวหลัก" button (dispatches `SET_ACTIVE_CREATURE`). Element color dot rendered next to name. All name displays across PartyGrid, VaultGrid, and HatchedGrid/CreatureCard updated to use `creatureName()`.
- **Name suggestions at hatch** (2026-06-15): `HatchOverlay.jsx` naming phase replaced — no more text input. Shows 5 large Thai name suggestion buttons (from `CREATURE_NAME_SUGGESTIONS[element]` in `creatureSystem.js`). Child taps one → `SET_CREATURE_NAME` + `doClose`. "ข้ามการตั้งชื่อ" still available. Elements: fire/water/thunder/nature/shadow/light → 5 names each.
- **CreatureDetailPopup family + friendship focus** (2026-06-15): `FAMILY_LABELS_TH` (16 families Thai labels) + `FAVSUBJ_TH/COLOR` constants. Header shows Moonborn badge when `family==='moon'`. Creature canvas has element `drop-shadow` glow. Name section: custom `creatureName` priority + family badge + element badge. Friendship stats row: days together (from `bornDate`), `adventuresWith`, `questionsAnswered`, favorite-subject color badge (green/blue/purple).
- **Home companion zone enlarged** (2026-06-15): companion zone height 52→80px, CreatureCanvas from 22→46px so party creature is more visible.
- **ECA relationship fields** (2026-06-15 — ECA-MVP-3): `adventuresWith`, `questionsAnswered`, `eggStartDate` added to all eggs at hatch (`StateContext HATCH_COMPLETE`) and backfilled by `_migrateBattleStats` for legacy creatures. `ADD_XP` increments `questionsAnswered` (+1) for active party creature on correct answer. `ROUND_COMPLETE` increments `adventuresWith` (+1) for active party creature each round.
- **HatchOverlay biography phase** (2026-06-15 — ECA-4): New `'bio'` phase renders before `'tapping'` when active party creature has `adventuresWith>0`. Separate portal shows: `CreatureCanvas` (100px, celebrate), creature name, adventure count, question count, "ฟักไข่ต่อ!" button → `setPhase('tapping')`, "ยังไม่ฟักตอนนี้" → doClose. `bioDNA` synthesized from stored stats for legacy creatures lacking `.dna` field.
- **Post-session egg growth banner** (2026-06-15 — ECA-3): `Home.jsx` checks `state.sessionXP>0` on mount → shows "ไข่ของเราโตขึ้นนะ!" (stage<5) or "อีกนิดเดียวก็ฟักแล้ว!" (stage≥5) banner with 900ms delay, auto-hides after 3s, plays stageUp tone. `SET_SESSION_XP:0` dispatched on mount to reset for next visit.
- **Phase 4: Voice layer** (2026-06-15): `playCreatureSound(voiceProfile, moment)` added to `audio.js`. Accepts `voiceProfile = buildVoiceProfile(dna)` + 5 moments: `pet`, `food`, `reunion`, `celebrate`, `sleep`, `ambient`. Pitch-shifted using `pitchBase × (1 ± pitchVariance)`. Sound families: `chirp` (default), `peep` (shy/small), `trill` (playful/curious), `hum` (brave/gentle), `squeak` (crystal/dragon). `Home.jsx` imports `buildVoiceProfile` from `creatureGenerator.js` + `playCreatureSound`. `voiceProfile` derived via `useMemo` from active party creature's DNA. Wired to: creature companion tap, reunion visit (>4h), companion celebrate/wave/sleep personality behaviors. Falls back to `playTone('chirp')` when no voice profile.
- **Bond unlock combat effects** (2026-06-15): `WorldBattle.jsx` `creatureStats` useMemo applies bond bonuses: bond≥25 → ATK×1.05; bond≥50 → SPD+30 (≈+15% dodge rate in the SPD/200 formula); bond≥100 → ATK×1.5. `onCorrect()` dispatches `CREATURE_HEAL(+1)` when bond≥75 (passive heal per correct answer).
- **6-creature hard limit** (2026-06-15): `HatchOverlay.jsx` checks `hatchedEggs.length >= 6` during tapping phase — shows a friendly "คลังเต็มแล้ว!" blocking panel (with count + back button) instead of the egg. Hard gate; no hatch possible with 6+ creatures.
- **Evolution animation toast** (2026-06-15): `state.pendingEvoNotice` (new field in `defaultState`) stores `{creatureId, newStage, creatureName}` when evoStage changes. Set in ADD_XP, ROUND_COMPLETE, INCREMENT_BATTLE_WINS reducers. `App.jsx` watches this via `useEffect` → `showToast("★ [name] วิวัฒนาการแล้ว! → [stage]")` → `CLEAR_EVO_NOTICE` dispatch.
...
- **Pixel Art Creature System — `creatureAlgorithm.js`** (2026-06-16): Used by BattleScreen, ChallengerOverlay, EggMemory. Old HSL-based procedural draw replaced with 6-element × 3-stage pixel art renderer. 12×12 grid, P=`floor(min(W,H)/12)` scale, centered with `(ox,oy)` offset; `#0a0a12` background. Element from stats: streak≥7→shadow, acc≥85→nature, thai-dominant→fire, math-dominant→water, eng-dominant→thunder, balanced→light. Stage from `stats.evoStage` ('baby'/'teen'/'final', default 'baby'). Baby: round form with element-specific ears/tail/cheeks/sides. Teen: wider body (8 units) + shoulder pads + dramatic ears + bigger tail. Final: crown/helmet, wide armored chest, glowing eyes for shadow/fire, element wings/aura. `getCreatureName(element, evoStage)` exported — Thai names per element×stage. `getCreatureSeed(egg)` unchanged.
