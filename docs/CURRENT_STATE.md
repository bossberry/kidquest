...
- **Pixel art rendering** — `drawCreature(canvas, seed, stats)` + `getCreatureSeed(egg)` from `creatureAlgorithm.js` — **single source of truth for all screens**
  ...
  - **Baby-stage silhouette redesign (block-attachment technique)**: all 4 baby types redesigned using Pokémon reference images. FURRED: 2×3 tall ear blocks, 10-wide head / 6-wide body chibi ratio, flat 3×2 tail rectangle, 4 separated leg stubs. WINGED: 3×4 rectangular wing blocks with consistent 2-wide fill (no taper), 2-stub bipedal legs, no ears. SCALED: 8×5 head with 2×3 sideways frills, 2×5 straight vertical tail, right leg at x=6 (gap before tail). CHITIN: 2-row antennae, head at y=2, 3-segment widening 6→8→10.
