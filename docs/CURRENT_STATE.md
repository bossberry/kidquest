...
- **Egg Companion Adventure MVP** (2026-06-04): Current egg is the player's avatar across all 3 Subject Adventure modes. MoveSelectBattleMode: EggCanvas with charge/lunge animations, combo glow ring (gold outline builds with streak), near-hatch continuous glow, sparkle on correct. DefenseMode: EggCanvas — child defends their own egg. ChaseMode: EggCanvas — egg dashes on correct. Props flow: `Session` pulls `eggStatsData`/`eggProgressData`/`readyToHatch` from context and passes to modes.
