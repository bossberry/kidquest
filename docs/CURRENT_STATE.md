...
_Last updated: 2026-07-07 (Room expansion UX: ghost room previews; room items simplified to monster drops + auto-collect + instant craft; enemy art polish pass)_
...
- **Auto-collect materials** (`state.materials: { flower, wood, stone, water, stardust, mushroom }`, all default 0; `state.dailyMaterialsCollected`/`state.lastMaterialDate` cap at 15/day, `todayStr()` reset convention): every committed step in `WorldScreen.jsx`'s `tryMove` has a 15% chance to award 1 material via `pickAutoMaterial(tileMap, col, row, theme)` — `T.TALL`→flower (mushroom in the forest theme), `T.PATH`→stone, standing next to a `T.TREE`→wood, `T.GRASS`→theme-dependent (sky→stardust, snow→water). **Deviation from the literal spec table** (same kind of correction the original crafting session made): `tileMaps.js`'s generators never place `T.WATER` in any world (grepped — the `W = T.WATER` const is defined but never used as a map cell value), so a literal "water tile adjacency" rule would be permanently dead; water is folded into the existing theme-dependent `T.GRASS` slot instead (paired with sky/stardust) so every material stays reachable in some world. Each material caps at 30. A quiet `collect_tick` SFX + a brief floating icon (reuses the `dmg-float` CSS keyframe, 300ms) give feedback; no notification when the daily cap blocks a roll (silent, per spec). `WorldHUD.jsx` shows a tiny collapsible material strip, only when the child owns any.
