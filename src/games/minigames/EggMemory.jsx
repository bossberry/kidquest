...
export default function EggMemory() {
  ...
  useEffect(() => {
    ...
    const pool = hatched.length >= 4
      ? hatched.slice(0, 8).map((e, i) => ({ id: i, type: 'creature', egg: e }))
      : ['🐱','🐕','🐟','🐦','🐸','🐘','🐰','🐷'].map((s, i) => ({ id: i, type: 'emoji', sym: s }))
    const pairCount = Math.min(6, Math.max(4, pool.length))
    const pairs = pool.slice(0, pairCount)
    setCards(shuffle([...pairs.map((p,i)=>({...p,key:i})), ...pairs.map((p,i)=>({...p,key:pairCount+i}))]))
  }, []) // eslint-disable-line
  ...
  if (done) return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:24, textAlign:'center' }}>
      ...
      <button onClick={() => { setDone(false); setFlipped([]); setMatched(new Set()); setMoves(0); setCards(shuffle([...cards])) }}
        style={{ width:'100%', background:'var(--purple)', color:'#fff', border:'none', borderRadius:10, padding:14, fontFamily:'Mitr,sans-serif', fontSize:16, fontWeight:600, cursor:'pointer' }}>🔄 เล่นอีกครั้ง</button>
    </div>
  )

  const pairCount = cards.length / 2
  const cols = pairCount <= 4 ? 2 : 3
  ...
  return (
    <div style={{ width:'100%', maxWidth:480, padding:16, fontFamily:'Mitr,sans-serif' }}>
      <div style={{ display:'flex', justifyContent:'space-between', marginBottom:12 }}>
        ...
        <span style={{ fontSize:14 }}>✅ {matched.size/2}/{pairCount}</span>
      </div>
      <div style={{ display:'grid', gridTemplateColumns:`repeat(${cols},1fr)`, gap:10 }}>
        {cards.map((card, idx) => {
          ...
          return (
            <MemCard key={idx} card={card} flipped={isFlipped} matched={isMatched} onClick={() => flip(idx)} />
          )
        })}
      </div>
    </div>
...
function MemCard({ card, flipped, matched, onClick }) {
  ...
  useEffect(() => {
    if ((flipped || matched) && canvasRef.current && card.type === 'creature') {
      drawCreature(canvasRef.current, getCreatureSeed(card.egg), card.egg.eggStats || {})
    }
  }, [flipped, matched]) // eslint-disable-line

  return (
    <div onClick={onClick} style={{ borderRadius:12, background: matched ? 'var(--green-l)' : 'var(--purple-l)', border: `2px solid ${matched ? 'var(--green)' : 'var(--purple)'}`, height:80, display:'flex', alignItems:'center', justifyContent:'center', cursor: matched ? 'default' : 'pointer', transition:'all .2s', overflow:'hidden' }}>
      {flipped || matched ? (
        card.type === 'creature'
          ? <canvas ref={canvasRef} width={60} height={60} style={{ borderRadius:8 }} />
          : <span style={{ fontSize:32 }}>{card.sym}</span>
      ) : (
