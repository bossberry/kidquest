...
- **Egg Home Redesign** (2026-06-11): Pokémon-style background + Tamagotchi ambient life. HomeBackground.jsx fully rebuilt — vivid FireRed/LeafGreen sky (`#4ec8f0→#87ddff→#c8f0ff→#d4f7c0`), sun with 4s pulse animation, larger CSS mountains, curved ground (`border-radius:50% 50% 0 0 / 30px`). Ambient layer: 2 CSS-drawn butterflies (wing flap via scaleY alternation, sine-path flight loop), 1 CSS bird (fast wing flap, 15s cross-screen), 3 leaf particles (fall + rotate + sway), 4 fireflies (night only, glow + drift), 6 flowers (day, float animation), ground mounds for depth. Night: vivid #0a1a3a dark-blue sky, moon+crescent, 12 twinkling stars, fireflies, magic particles. Home.jsx UI: title "ไข่ของ{name}" moved above egg (smaller, 17px, soft glow text-shadow); stage dots replaced with single mood emoji (😊/🤩/😋/😴 based on eggAnim state); item tray now glassmorphism inner container (`rgba(255,255,255,0.15)` + `backdropFilter:blur(8px)` + frosted border); ออกสำรวจ! button has shimmer animation (`home-shimmer` keyframe, gradient sweep); egg has ground shadow (`radial-gradient(ellipse, rgba(0,0,0,0.18), transparent)` underneath canvas).
...
- **`src/lib/drawEnemy.js`** (NEW): Canvas sprite renderer. `drawEnemy(ctx, enemyType, size, x, y)` — 7 types: `sleepy_bunny/bunny`, `bouncy_slime/slime`, `fox_kit/fox`, `egg_pawn`, `leaf_sprite`, `grumpy_mole`, `mushroom_imp`. 48-unit design space scaled to any pixel size. x,y offset for world canvas; x=0,y=0 for dedicated battle canvas.
