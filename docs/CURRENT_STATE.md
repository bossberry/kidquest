...
- **`src/components/HatchOverlay.jsx`**: After 5-tap hatch reveal, shows element badge (colored pill) + two buttons: "ตั้งชื่อ" → 'naming' phase or "ข้ามการตั้งชื่อ". Naming phase: text input (max 16 chars) + confirm → dispatches `SET_CREATURE_NAME` for newest hatchedEgg. "ข้ามการตั้งชื่อ" available in both done and naming phases.
