...
- **Battle Subject Routing** (2026-06-11) — `src/lib/battleSubject.js` (NEW): `getBattleSubject(enemyType, sessionLog)` 3-layer selection: exploring subject wins → enemy preferred if comfortable → weakest by rank. `src/lib/subjectReadiness.js` (NEW): shared `computeReadiness` extracted from Report.jsx. `ENEMY_DATA` has `subject` field for all 7 types. WorldScreen uses `getBattleSubject` instead of `getWeakestSubject`. All 3 subjects (thai/math/eng) now route correctly.
