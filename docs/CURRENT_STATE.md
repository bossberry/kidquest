...
- **Battle Subject Routing** (2026-06-11) — `src/lib/battleSubject.js`: `getBattleSubject(sessionLog, state)` sorts by readiness tier (exploring→comfortable→notready→strong), rotates ties via `dailyBattleRounds`. `getBattleLevel(subject, state)` derives level from XP (`floor(xpX/120)+1`, clamped to LEVELS max id). Enemy type has no influence on subject selection. `src/lib/subjectReadiness.js`: shared `computeReadiness`. `WorldScreen.triggerBattle` uses both functions.
