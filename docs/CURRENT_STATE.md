...
- **Enemy death animation** (2026-06-11): Enemies have `dead/deathTimer/opacity` state fields. On returning from battle, `sessionStorage 'kq_last_battle'` is read on enemy init — first matching type enemy is set `dead: true, deathTimer: 180`. In `updateEnemies()` dead enemies decrement `deathTimer`, fade `opacity = timer/180`, get removed at 0. `renderEnemies()` draws dead enemies as squished rotated corpse (`scale(1, 0.3)`, rotated 90°) + ✕ mark above, fading with `globalAlpha = e.opacity`.
