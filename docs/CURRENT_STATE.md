...
- **`src/components/Home.jsx`**: Party HP bar now shows: element color dot (6×6px colored square bottom-right of creature canvas), bond meter (gold bar, 3px height, active creature only), creature's `creatureName || creature.n` for display name. Header now shows element hint badge at stage 2+ ("ธาตุน้ำ?" in element color with transparent bg). Imports: `getEggElementHint`, `CREATURE_ELEMENT_COLORS`, `EVO_STAGE_LABELS_TH`.
