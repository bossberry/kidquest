...
## 2026-07-08 — Room screen: full-bleed canvas, mini-map overlay removed

Follow-up feedback fixes on top of the 2026-07-07 scrollable-room work.

### src/components/Room.jsx
- **Fix 1 (full-bleed room)**: added a Room.jsx-local `computeFullBleedGeometry(W, H)`, used only for the main room's draw geometry — deliberately NOT a change to the shared `computeRoomGeometry()` in `roomScene.js` (that function is also used by `DecoratedRoom.jsx`/`RoomVisit.jsx`/the FriendsScreen thumbnail, which must keep their existing fit-within-bounds framing). `computeRoomGeometry` takes the MIN of the width/height scale budgets so the whole iso shape always fits inside the canvas — on a canvas much wider than the iso room's natural ~10:8 aspect ratio, that leaves large empty margins on the sides (the reported bug). Tried taking the MAX instead first; verified live at a wide desktop viewport that it overcorrected badly — the room scaled up so far the walls scrolled entirely off-screen, leaving just a floor. Settled on **averaging** the width-fit and height-fit scales instead, which trims the margin substantially on the generous axis while only moderately cropping the tight one. Verified live at both a ~1200×650 and ~1500×620 viewport — walls stay visible, dark corner margins are much smaller, ghost/real neighbor rooms still peek in at the edges and pan into full view correctly, and slot tap-to-place hit-testing still lines up with the new geometry.
- **Fix 2 (remove mini-map)**: deleted the `RoomMiniMap` component and its render entirely — panning to a revealed ghost/adjacent room is now the only room-navigation affordance. Side effect worth flagging: this also removed the only UI entry point for `SET_HOME_ROOM` ("ตั้งเป็นห้องหลัก" — choosing which room shows on Home's background), since that button lived inside the same mini-map overlay. That's a real, working, unrelated feature that's now unreachable from the UI — not something this task specifically asked to remove, so calling it out rather than leaving it a silent regression.
- **Fix 3 (remove craft button) — NOT done, stale premise**: the spec assumed crafting is reached by tapping a placed `crafting_table` furniture item, with the ⚒️ button as a redundant shortcut to remove. Checked `src/lib/roomItems.js` first: `crafting_table` was already removed from the catalog entirely in the 2026-07-07 "monster drops + auto-collect + instant craft" session, which replaced the whole workbench-item flow with the ⚒️ button as crafting's *only* entry point. Removing the button as asked would have left literally no way to craft in the game. Left the button in place; flagging for a decision rather than silently breaking crafting or silently reintroducing the removed workbench item.
