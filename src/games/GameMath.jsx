...
function MathLevelGame({ lv, onBack }) {
  ...
  const next = () => {
    ...
    if (cur+1>=10){
      ...
      dispatchAddCoins(dispatch, _mathCoins)
      ...
      if (lv?.isFoundation) {
        ...
        if(cur2<8){dispatch({type:ACTIONS.UNLOCK_LEVEL,payload:{world:'math',newLevel:cur2+1}});dispatchAddCoins(dispatch, 15, `math_${cur2+1}`);showToast(`✨ ปลดล็อก Level ${cur2+1}!`);spawnConfetti(15);playTone('unlock')}
      }
