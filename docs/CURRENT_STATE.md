...
- **Home 2.0 — Adventure Director**: `Home.jsx` restructured. Single `⭐ ผจญภัยต่อ` recommendation card (deterministic: hatch → shop-first-run → weakest-subject). Subject grid now behind collapsible "อยากเลือกเอง?" toggle (closed by default) — `subjectsOpen` useState. Shop Mission card removed from Home (Shop still reachable via Continue Adventure recommendation when `shopV1.runs === 0`). 2×2 minigame grid replaced with `🎁 เซอร์ไพรส์วันนี้` single-event rotation (date-hash of unlocked minigames; played-today check via `sessionLog`; teaser card when none unlocked). `getRecommendation()` and `getSurpriseEvent()` helpers (no new state).
