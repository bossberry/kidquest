...
function PhonicsGame({ lv, onBack }) {
  ...
  const next=()=>{playTone('next');if(cur+1>=10){setDone(true);const p=score/10;const _c1=Math.max(2,Math.min(12,Math.round(12*(p<0.5?0.3:p)*(1-(state.levelMastery?.eng?.[1]||0)))));setCoinsEarned(_c1);dispatchAddCoins(dispatch, _c1);showItemToast(`🪙 +${_c1}`);dispatch({type:ACTIONS.ROUND_COMPLETE,payload:{streak,score:p}});dispatch({type:ACTIONS.UPDATE_LEVEL_MASTERY,payload:{world:'eng',levelId:1,value:p*0.4+((state.levelMastery?.eng?.[1])||0)*0.6}});if(p>=0.8){const cur2=state.subjectLevels?.eng||1;if(cur2<4){dispatch({type:ACTIONS.UNLOCK_LEVEL,payload:{world:'eng',newLevel:cur2+1}});dispatchAddCoins(dispatch, 15, `eng_${cur2+1}`);showToast(`✨ Level ${cur2+1} Unlocked!`);spawnConfetti(15);playTone('unlock')}}if(p>=0.9){playTone('fanfare');spawnConfetti(30)}else if(p>=0.8){playTone('complete')}dispatch({type:ACTIONS.LOG_SESSION,payload:{ts:sessionStart.current,world:'eng',missionId:null,level:1,dur:Date.now()-sessionStart.current,score:p,wrong:10-score,hints:0,completed:p>=0.8,nextAction:null,phaseStats:null}})}else{setAnswered(false);setFeedback(null);setCur(c=>c+1)}}
...
function CVCGame({ lv, onBack }) {
  ...
  const next=()=>{playTone('next');if(cur+1>=10){setDone(true);const p=score/10;const _c2=Math.max(2,Math.min(12,Math.round(12*(p<0.5?0.3:p)*(1-(state.levelMastery?.eng?.[2]||0)))));setCoinsEarned(_c2);dispatchAddCoins(dispatch, _c2);showItemToast(`🪙 +${_c2}`);dispatch({type:ACTIONS.ROUND_COMPLETE,payload:{streak,score:p}});dispatch({type:ACTIONS.UPDATE_LEVEL_MASTERY,payload:{world:'eng',levelId:2,value:p*0.4+((state.levelMastery?.eng?.[2])||0)*0.6}});if(p>=0.8){const c2=state.subjectLevels?.eng||1;if(c2<4){dispatch({type:ACTIONS.UNLOCK_LEVEL,payload:{world:'eng',newLevel:c2+1}});dispatchAddCoins(dispatch, 15, `eng_${c2+1}`);showToast(`✨ Level ${c2+1} Unlocked!`);spawnConfetti(15)}}if(p>=0.9){playTone('fanfare');spawnConfetti(30)}dispatch({type:ACTIONS.LOG_SESSION,payload:{ts:sessionStart.current,world:'eng',missionId:null,level:2,dur:Date.now()-sessionStart.current,score:p,wrong:10-score,hints:0,completed:p>=0.8,nextAction:null,phaseStats:null}})}else{setAnswered(false);setFeedback(null);setCur(c=>c+1)}}
  if(done)return<ResultScreen score={score} total={10} xp={xp} streak={streak} coins={coinsEarned} onReplay={()=>{sessionStart.current=Date.now();setCur(0);setScore(0);setStreak(0);setXp(0);setDone(false);setAnswered(false);setFeedback(null);setCoinsEarned(0)}} onBack={onBack}/>
  if(!q)return null
  return(
    <div style={{width:'100%',maxWidth:480,padding:'8px 0'}}>
...
function SightGame({ lv, onBack }) {
  ...
  const next=()=>{playTone('next');if(cur+1>=qs.length){setDone(true);const p=score/qs.length;const _c3=Math.max(2,Math.min(12,Math.round(12*(p<0.5?0.3:p)*(1-(state.levelMastery?.eng?.[3]||0)))));setCoinsEarned(_c3);dispatchAddCoins(dispatch, _c3);showItemToast(`🪙 +${_c3}`);dispatch({type:ACTIONS.ROUND_COMPLETE,payload:{streak,score:p}});dispatch({type:ACTIONS.UPDATE_LEVEL_MASTERY,payload:{world:'eng',levelId:3,value:p*0.4+((state.levelMastery?.eng?.[3])||0)*0.6}});if(p>=0.8){const c2=state.subjectLevels?.eng||1;if(c2<4){dispatch({type:ACTIONS.UNLOCK_LEVEL,payload:{world:'eng',newLevel:c2+1}});dispatchAddCoins(dispatch, 15, `eng_${c2+1}`);showToast(`✨ Level ${c2+1} Unlocked!`);spawnConfetti(15)}}if(p>=0.9){playTone('fanfare');spawnConfetti(30)}dispatch({type:ACTIONS.LOG_SESSION,payload:{ts:sessionStart.current,world:'eng',missionId:null,level:3,dur:Date.now()-sessionStart.current,score:p,wrong:qs.length-score,hints:0,completed:p>=0.8,nextAction:null,phaseStats:null}})}else{setAnswered(false);setFeedback(null);setCur(c=>c+1)}}
